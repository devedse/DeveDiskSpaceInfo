...
namespace DeveDiskSpaceInfo.Services
{
    ...
    {
        public static void OutputResults(AnalysisResult result, ShowOptions options)
        {
            if (options.JsonOutput)
            {
                OutputAsJson(result, options);
            }
            else
            {
                OutputAsText(result);
        ...
        private static void OutputAsJson(AnalysisResult result, ShowOptions options)
        {
            ...
            Console.WriteLine(JsonSerializer.Serialize(jsonResult, SfdiskJsonContext.Default.JsonOutputResult));
        }

        private static void OutputAsText(AnalysisResult result)
        {
            if (!result.Success)
            {
